...
export function TutoringMentorship() {
      ...
      const mockMentorApps: MentorApplication[] = [
        {
          ...
          experience: 8,
          specialties: ['Software Development', 'Career Guidance', 'Technical Leadership'],
        ...
        },
        {
          ...
          experience: 5,
          specialties: ['Financial Analysis', 'Investment Strategy', 'Career Development'],
          linkedinProfile: 'https://linkedin.com/in/ewilson',
          status: 'approved',
          reviewedBy: 'Super Admin',
          reviewerNotes: 'Strong background in finance, great for business students',
  ...
  const handleApproveTutor = (applicationId: string) => {
    setTutorApplications(prev => prev.map(app => 
  ...
  const handleApproveMentor = (applicationId: string) => {
    setMentorApplications(prev => prev.map(app => 
      app.id === applicationId ? { 
        ...app, 
        status: 'approved' as const,
        reviewedBy: 'Admin',
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <h1 className="text-3xl font-bold custom-font">Tutoring & Mentorship</h1>
        </div>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{tutorStats.total + mentorStats.total}</div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{tutorStats.pending + mentorStats.pending}</div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{tutorStats.approved + mentorStats.approved}</div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Active Tutors/Mentors</CardTitle>
            <Star className="h-4 w-4 text-campus-orange" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{tutorStats.approved + mentorStats.approved}</div>
          </CardContent>
                ...
                <TableBody>
                  {filteredTutorApps.map((application) => (
                    <TableRow key={application.id}>
                      ...
                      <TableCell>
                        {application.hourlyRate ? `$${application.hourlyRate}/hr` : 'Not specified'}
                      </TableCell>
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            ...
                            {application.status === 'pending' && (
                              <>
                                <DropdownMenuItem onClick={() => handleApproveTutor(application.id)}>
                                  <CheckCircle className="mr-2 h-4 w-4" />
                                  Approve
                                </DropdownMenuItem>
                                <DropdownMenuItem onClick={() => handleRejectTutor(application.id)}>
                      ...
                      <TableCell className="text-right">
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            ...
                            {application.status === 'pending' && (
                              <>
                                <DropdownMenuItem onClick={() => handleApproveMentor(application.id)}>
                                  <CheckCircle className="mr-2 h-4 w-4" />
                                  Approve
                                </DropdownMenuItem>
                                <DropdownMenuItem onClick={() => handleRejectMentor(application.id)}>
                                  <XCircle className="mr-2 h-4 w-4" />
                                  Reject
                                </DropdownMenuItem>
                              </>
