...
interface TutoringRequestDetailsModalProps {
  open: boolean;
...
export const TutoringRequestDetailsModal = ({
  open,
  onOpenChange,
  request,
  isTutor,
}: TutoringRequestDetailsModalProps) => {
  ...
  const updateStatusMutation = useMutation({
    ...
    }) => updateTutoringRequestStatus(requestId, status, responseMessage),
    onSuccess: () => {
  ...
  const isReceived = isTutor && request.status === "pending";
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle>Tutoring Request Details</DialogTitle>
          <DialogDescription>
            {isTutor
              ? "Review and manage this tutoring request"
              : "View your tutoring request details"}
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-medium">
                {isTutor ? "Requester" : "Tutor"}
              </p>
              <p className="text-sm text-muted-foreground">{participantName}</p>
              {isTutor && request.requester_department && (
