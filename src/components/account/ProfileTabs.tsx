...
export const ProfileTabs = ({ 
  ...
  const getTabs = () => {
    const baseTabs = [
      { id: 'posts', label: 'Posts', icon: MessageSquare, count: user.posts.length },
    ];

    // Only show other tabs for own profile
    if (isOwnProfile) {
      baseTabs.push({ id: 'likes', label: 'Likes', icon: Heart, count: 0 });

      // Add role-specific tabs
      if (hasRole(authUser, 'student')) {
        baseTabs.push({ id: 'groups', label: 'Groups', icon: Users, count: user.joinedGroups.length });
      }

      if (hasRole(authUser, 'tutor')) {
        baseTabs.push({ id: 'tutoring', label: 'Tutoring', icon: BookOpen, count: user.tutoringRequests.length });
      }

      if (hasRole(authUser, 'mentor')) {
        baseTabs.push({ id: 'mentorship', label: 'Mentorship', icon: GraduationCap, count: null });
      }

      baseTabs.push({ id: 'settings', label: 'Settings', icon: Settings, count: null });
    }
      ...
      <TabsContent value="posts" className="space-y-0 mt-0">
        {user.posts.length > 0 ? (
          ...
          <div className="text-center py-12 text-muted-foreground">
            ...
            <p className="text-sm">When {isOwnProfile ? 'you' : user.displayName} post something, it will show up here.</p>
          </div>
        )}
      </TabsContent>
        ...
        <>
          <TabsContent value="likes" className="space-y-4 mt-0">
            ...
            <div className="text-center py-12 text-muted-foreground">
              ...
              <p className="text-sm">Tap the heart on posts you like and they'll show up here.</p>
            </div>
          </TabsContent>
            ...
            <TabsContent value="groups" className="space-y-4 mt-0">
              ...
              {user.joinedGroups.length > 0 ? (
                ...
                <div className="text-center py-12 text-muted-foreground">
                  ...
                  <p className="text-sm">Join groups to connect with others who share your interests.</p>
                </div>
              )}
            </TabsContent>
          )}
            ...
            <TabsContent value="tutoring" className="space-y-4 mt-0">
              ...
              {user.tutoringRequests.length > 0 ? (
                ...
                <div className="text-center py-12 text-muted-foreground">
                  ...
                  <p className="text-sm">Your tutoring sessions will appear here.</p>
                </div>
              )}
            </TabsContent>
          )}
          ...
          {hasRole(authUser, 'mentor') && (
            <TabsContent value="mentorship" className="space-y-4 mt-0">
              ...
              <div className="text-center py-12 text-muted-foreground">
                ...
                <p className="text-sm">Your mentoring activities will show up here.</p>
              </div>
            </TabsContent>
          )}
              ...
              <CardContent className="p-6 space-y-6 rounded-none">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">Customize how Campus Vibe looks on your device</p>
                  </div>
                  <ThemeToggle />
                </div>
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">Get notified about activity via email</p>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">Control your privacy and data settings</p>
                  </div>
