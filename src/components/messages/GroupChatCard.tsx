...
export const GroupChatCard = ({ groupChat, isSelected, onClick }: GroupChatCardProps) => {
  return (
    <div
      onClick={onClick}
          ...
          <div className="flex items-center justify-between mb-1">
            <div className="flex items-center space-x-2 min-w-0">
              ...
              {groupChat.isModerator && !groupChat.isAdmin && (
                <Badge variant="outline" className="text-xs px-1 py-0">Mod</Badge>
              )}
            </div>
            <span className="text-sm text-muted-foreground flex-shrink-0 ml-2">{groupChat.timestamp}</span>
          </div>
          <div className="flex items-center justify-between">
            <div className="flex-1 min-w-0">
              <p className="text-sm text-muted-foreground truncate pr-2">{groupChat.lastMessage}</p>
              <p className="text-xs text-muted-foreground mt-1">{groupChat.memberCount} members</p>
            </div>
